...
# Check if OP_PROPOSER_GAME_FACTORY_ADDRESS environment variable is set
if [ -z "$OP_PROPOSER_GAME_FACTORY_ADDRESS" ]; then
  # If not set, check if the file exists
  if [ ! -f "$DEPLOYMENT_DIR/addresses.json" ]; then
    echo "File $DEPLOYMENT_DIR/addresses.json does not exist. Please import data/deployments or set the OP_PROPOSER_GAME_FACTORY_ADDRESS variable."
    exit 1
  fi
